<?php
  ...
  $sql = 
   "
   select 음식점.*,avg(리뷰.별점) as '평점' from 음식점, 리뷰 where 리뷰.음식점ID=음식점.음식점ID and 음식점.음식점ID=any(select 음식점ID from 분류 where 카테고리='".$카테고리."') group by (음식점.음식점ID) order by 평점 desc";
